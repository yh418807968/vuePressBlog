...
则服务器会带上CORS头信息返回响应（），浏览器发现有CORS则知道被允许了，按正常响应处理；若没有CORS信息，则抛出错误。
### 复杂请求
#### 预检请求
先预检请求options：带上origin，Access-Control-Request-Method，Access-Control-Request-Headers
服务端收到预检请求，检查以上字段后，若允许则带上CORS的相关头信息作出回应；若拒绝，则返回一个普通的http，没有CORS信息。浏览器接收到后，发现没有CORS则会认为服务器不同意，便出发一个错误，最终通过控制台打印出来。
#### 正常请求
通过预检请求后，浏览器会进行正常的请求，此时就会和一个简单请求一样，带上origin字段，服务器会返回
### 验证
```js
...
> 若在浏览器看不到OPTIONS请求，则通过以下步骤可以看到OPTIONS方法：
> 1、新标签页输入`chrome://flags/#out-of-blink-cors`
> 2、将Out of blink CORS设置为disabled
> 3、重启chrome
...
```
设置target为目标服务器地址，当本地发起ajax请求时，请求会被发到localhost上，localhost直接与浏览器交互，其Access-Control-Allow-Origin为*；然后localhost将请求转换给`your-api-server:port`，接收到响应后再转发给浏览器。
若服务器上
