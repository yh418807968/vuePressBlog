...
假设请求的资源未html文件，并且请求成功，则客户端收到响应后，开始渲染页面。
> 接收到响应后，若是由其他页面跳转过来，则地址栏会更新为新页面的信息，history tab会更新，之后可通过返回键返回导航来的页面。

## 渲染
> 浏览器进程将网络进程接收到的 HTML 数据提交给渲染进程，由渲染进程负责渲染页面
...
> 在此过程中，
> * 如果遇到img、link等标签，会通过网络进程对相关资源进行下载
> * 如果遇到script标签时，渲染进程会停止解析 HTML，而去加载、解析和执行 JS 代码。可以在标签上添加了 async 或 defer 等属性，浏览器会异步的加载JS 代码，而不会阻塞渲染
